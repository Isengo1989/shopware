...
class Blog extends Resource
{
    ...
    {
        return $this->getManager()->getRepository(BlogModel::class);
    }
    ...
    public function getOne($id)
    {
        ...
        /** @var array $blog */
        $blog = $query->getOneOrNullResult($this->getResultMode());

        if (!$blog) {
            throw new ApiException\NotFoundException(sprintf('Blog by id %d not found', $id));
        }
     ...
     * @return array
     */
    public function getList(array $categoryIds = [], $offset = 0, $limit = 25, array $criteria = [], array $orderBy = [])
    {
        $this->checkPrivilege('read');

        $query = $this->getRepository()->getListQuery($categoryIds, $offset, $limit, $criteria, $orderBy);
